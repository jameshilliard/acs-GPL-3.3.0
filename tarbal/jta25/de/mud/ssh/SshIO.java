 ...
 * @version $Id: SshIO.java,v 1.2 2003/04/22 18:26:38 jennifer Exp $
 */
public abstract class SshIO {
  ...
  synchronized public void sendData(String str) throws IOException {
	...
	/* Concatenates bytes to bytes */
	private byte[] concatBytes(byte result[], byte resultPacket[]) {
		byte temp[];
		temp = new byte[result.length + resultPacket.length];
		System.arraycopy(result, 0, temp, 0, result.length);
		System.arraycopy(resultPacket, 0, temp, result.length, resultPacket.length);
		return temp;
	}
  ...
  public byte[] handleSSH(byte buff[])
    ...
    if (currentpacket.isFinished()) {
      if (useprotocol == 1) {
				resultPacket = handlePacket1((SshPacket1) currentpacket);
        if (result == null) 
          result = resultPacket;
        else 
					result = concatBytes(result, resultPacket);
        currentpacket = new SshPacket1(crypto);
      } else {
				resultPacket = handlePacket2((SshPacket2) currentpacket);
        if (result == null) 
          result = resultPacket;
        else 
					result = concatBytes(result, resultPacket);
  ...
  private byte[] handlePacket2(SshPacket2 p)
    throws IOException {
    switch (p.getType()) {
      ...
      case SSH2_MSG_DISCONNECT:
        ...
        byte[] discreason1 = p.getStringBytes();
      ...
      default:
        return ("SSH2: handlePacket2 Unknown type " + p.getType()).getBytes();
    }
    return "".getBytes();
  }

  private byte[] handlePacket1(SshPacket1 p)
    ...
    switch (p.getType()) {
      ...
      case SSH_SMSG_SUCCESS:
        ...
        if (lastPacketSentType == SSH_CMSG_SESSION_KEY) {
          //we have successfully sent the session key !! (at last :-) )
          Send_SSH_CMSG_USER();
          break;
        }
