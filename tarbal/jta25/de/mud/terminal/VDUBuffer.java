 ...
 * @version $Id: VDUBuffer.java,v 1.3 2002/08/22 09:40:44 mrdon Exp $
 */
public class VDUBuffer {

  /** The current version id tag */
  public final static String ID = "$Id: VDUBuffer.java,v 1.3 2002/08/22 09:40:44 mrdon Exp $";
  ...
  protected Set bufferListeners = new HashSet();
  protected BufferListener[] cachedListeners;
  protected boolean listenersStale;
...
*/
    fireBufferEvent(new BufferEvent(BufferEvent.WINDOW_BASE_CHANGED, windowBase));
  }
  ...
  public void setScreenSize(int w, int h) {
    ...
    update[0] = true;
    fireBufferEvent(new BufferEvent(BufferEvent.SCREEN_SIZE_CHANGED, height));
    /*  FIXME: ???
  ...
  public void addBufferListener(BufferListener bl) {
      synchronized (bufferListeners) {
          bufferListeners.add(bl);
      }
      listenersStale = true;
  }
  
  public void removeBufferListener(BufferListener bl) {
      synchronized (bufferListeners) {
          bufferListeners.remove(bl);
      }
      listenersStale = true;
  }
  
  public BufferListener[] getBufferListeners() {
      if (listenersStale || cachedListeners == null) {
          synchronized (bufferListeners) {
              cachedListeners = (BufferListener[])bufferListeners.toArray(new BufferListener[]{});
          }
  ...
  public void fireBufferEvent(BufferEvent be) {
      BufferListener[] l = getBufferListeners();
      for (int x=0; x<l.length; x++) {
          l[x].handleBufferEvent(be);
      }
  }
}
