 ...
 *
 * @version $Id: vt320.java,v 1.2 2004/06/04 17:53:29 mehul Exp $
 * @author  Matthias L. Jugel, Marcus Meiﬂner
 */
public abstract class vt320 extends VDUBuffer implements VDUInput {
  /** The current version id tag.<P>
   * $Id: vt320.java,v 1.2 2004/06/04 17:53:29 mehul Exp $
   */
  public final static String ID = "$Id: vt320.java,v 1.2 2004/06/04 17:53:29 mehul Exp $";
  ...
  public vt320(int width, int height) {
    ...
    setIBMCharset(false);
    setTerminalID("vt320");
    //set buffer size default to 1000 lines
    setBufferSize(1000);
    //setBorder(2, false);
  ...
  /* For WinEMS: UTF8 Encoding 
	 Case 1: Translating 110xxxxx 10xxxxxx -> 00000xxx xxxxxxxx 
	 Case 2: Translating 1110xxxx 10xxxxxx 10xxxxxx -> xxxxxxxx xxxxxxxx */
  private final static int TSTATE_DATA_ENC2B = 17;		/* This state is for encoding Case 1 */
  private final static int TSTATE_DATA_ENC3B1 = 18;		/* This state is for encoding Case 2 */
  private final static int TSTATE_DATA_ENC3B2 = 19;		/* This state is for encoding Case 2 */
  private final static int TSTATE_DATA_ENCODED = 20;	/* This state means it is done encoding */
  ...
  public void keyTyped(int keyCode, char keyChar, int modifiers) {
    ...
    if (alt) {
			if (getTerminalID().equals("vt-utf8"))
				write("\u001b\u0001" + keyChar);
			else
    ...
    if (((keyCode == KeyEvent.VK_ENTER) || (keyChar == 10))
            && !control) {
      //write("\n", false); //Henrique: eliminate extra prompt.
      write("\r", false);
  ...
  private void putChar(char c, boolean doshowcursor) {
	...
	// Edson 4.16.03  
  //    System.out.println("term_state:" + term_state + " char:" + (int) c);
    switch (term_state) {
			/* Encoding UTF8. 
				 When 2nd byte is between 10 000000 - 10 111111 */
			case TSTATE_DATA_ENC2B: 
				if (c >= '\u0080' && c <= '\u00bf') {
					c = (char) (WINvar + (c & '\u003f'));
					term_state = TSTATE_DATA_ENCODED;
				} else {
					term_state = TSTATE_DATA;
				}
				putChar(c, doshowcursor);
				break;
			case TSTATE_DATA_ENC3B1:
				/* If 2nd byte is between 1000 0000 - 1011 1111, AND it with 
						11 1111, so that first two bit remains 10, and shift 6 space. */
				if (c >= '\u0080' && c <= '\u00bf') {
					WINvar = (char)(WINvar + ((c & '\u003f') << 6));
					term_state = TSTATE_DATA_ENC3B2;
					break;
				}
				term_state = TSTATE_DATA;
				putChar(c, doshowcursor);
				break;
			case TSTATE_DATA_ENC3B2:
				if (c >= '\u0080' && c <= '\u00bf') {
					c = (char) (WINvar + (c & '\u003f'));
					term_state = TSTATE_DATA_ENCODED;
				} else {
      ...
      case TSTATE_DATA:
        ...
        switch (c) {
          ...
          default:
            {
              int thisgl = gl;
							/* Edson 4.16.03 - for Windows utf8 Encoding */
            	if (terminalID.equals("vt-utf8")) {
								/* utf8 encoding: 12-16 bits 
									If first byte is between e0 and ef (1110 0000 - 1110 1111), 
									change state to signal received first byte */
								if (c >= '\u00e0' && c <= '\u00ef') {	 
									term_state = TSTATE_DATA_ENC3B1;
									WINvar = (c & '\u000f') << 12;
									break;
								}

								/* utf8 encoding: 8-11 bits 
									If first byte is between 110 00000 - 110 11111, 
									change state. */
								if (c >= '\u00c0' && c <= '\u00df') { 
									term_state = TSTATE_DATA_ENC2B;
									WINvar = (c & '\u001f') << 6;
									break;
								}
							}
      ...
      case TSTATE_CSI:
        ...
        switch (c) {
          ...
          case 'c':/* send primary device attributes */
            ...
            if (terminalID.equals("vt-utf8")) subcode = "61;";
