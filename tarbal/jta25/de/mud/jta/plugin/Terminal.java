 ...
 *
 * @version $Id: Terminal.java,v 1.7 2004/06/19 00:25:07 jennifer Exp $
...
public class Terminal extends Plugin
  ...
  public static boolean sac = false;
  ...
  public Terminal(final PluginBus bus, final String id) {
    ...
    // create terminal
    terminal = new SwingTerminal(this, emulation);
    ...
    while (n >= 0)
      try {
        ...
        if (n > 0) {
			...
			if (n >= 10) {
					// more of the task list has "\r\n"
            		if ((new String(b, 0, n, encoding).substring(n-10, n-2)).equals("more----") == true) {
              			more = true;
					}
			} 

			if (n >= 8) {
					// more of the kernel log does not have "\r\n"
            		if ((new String(b, 0, n, encoding).substring(n-8, n)).equals("more----") == true) {
              			more = true;
					}
			} 

			if (n >= 4) {
				if ((new String(b, 0, n, encoding).substring(n-4, n)).equals("SAC>") == true) {
					sac = true;
				}
			}
...
//        if (n > 0) emulation.putString(new String(b, 0, n, encoding));
  ...
  public void breakn() {
	if (Applet.breakSeq == null) {
        	bus.broadcast(new TelnetCommandRequest((byte) 243));
	} else {
		try {
  ...
  public void disconnect() {
	...
	if (Applet.UnitType != null) {
		java.applet.AppletContext ctx = Applet.app.getAppletContext();
		System.out.println("old ctx= " + ctx);
        	System.out.println("APPLET = " + getJApplet(terminal));
       	 	System.out.println("context = " + getJApplet(terminal).getAppletContext()); 	
		try {
			if (Applet.UnitType != null && Applet.UnitType.equals("apm"))
				getJApplet(terminal).getAppletContext().showDocument(new URL(Applet.app.getCodeBase(), "usermode/close.html"));
			else
				getJApplet(terminal).getAppletContext().showDocument(new URL(Applet.app.getCodeBase(), "close.asp"));
		} catch (Exception e) {
			System.out.println("Error: unable to show doc");
        	}
	}
  }
  ...
  public void print() {
        ...
        //pj.setPrintable(this.appletFrame);
            //pj.setPrintable(appletFrame.getContentPane());
                    //pj.setP
              //pj.printDialog();
         //} catch (Exception e) {
        //    System.out.println("Error: unable to print using dialog");
        Frame f = getFrame(terminal);
        PrintJob printJob = terminal.getToolkit().getPrintJob(f, "JTA Terminal", null);
        //((Component) pluginLoader.getComponents().get("Terminal")).print(printJob.getGraphics());
        terminal.print(terminal.getGraphics());
        printJob.end();
   }
