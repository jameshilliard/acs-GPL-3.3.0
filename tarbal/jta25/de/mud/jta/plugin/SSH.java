 ...
 *
 * @version $Id: SSH.java,v 1.5 2004/06/19 00:23:44 jennifer Exp $
    ...
    bus.registerPluginListener(new OnlineStatusListener() {
     public void online() {
		...
		if(pass == null) {
			...
			//final TextField login = new TextField(user, 10); [JC]
			...
			ActionListener enter = new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					//System.out.println("*****PORT *******: " + Applet.portChosenSsh);
					//[RK]Mar/03/05 - if NatClustering/ssh - portChosenSsh == slave TCP port 
					if (Applet.portChosenSsh != null && Applet.portChosenSsh.equals("22") == false) {
						handler.setLogin(login.getText() + ":" + Applet.portChosenSsh);
					} else {
					...
					if (Applet.noAuthPopup == true) {
    	        		String loginStr;
						if (Applet.consoleChosen != null) {
							loginStr = login.getText() + ":" + Applet.consoleChosen;
						} else {
							loginStr = login.getText();
						}
						handler.setLogin(loginStr);
					}
    ...
    if(n > 0) {
      ...
      if(debug > 0 && buffer != null && buffer.length > 0)
				for (i=0; i<buffer.length; i++)
					System.err.println("ssh: buffer[" + i + "]: "+buffer[i]);
