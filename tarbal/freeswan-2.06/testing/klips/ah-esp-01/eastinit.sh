ipsec setup start

/testing/pluto/bin/wait-until-policy-loaded

# this should get added to the conn, ultimately
ipsec whack --label "\"SAwest-east\" leftrsasigkey"  --keyid "@west" --pubkeyrsa ""
ipsec whack --label "\"SAwest-east\" rightrsasigkey"  --keyid "@east" --pubkeyrsa ""
ipsec whack --name SAwest-east --encrypt --pfs --authenticate --rsasig --host "192.1.2.23"  --nexthop "192.1.2.45" --updown "ipsec _updown" --id "@east" --to --host "192.1.2.45" --nexthop "%direct" --updown "ipsec _updown" --id "@west" --ipseclifetime "28800" --rekeymargin "540" --keyingtries "1"
echo end eastinit.sh
