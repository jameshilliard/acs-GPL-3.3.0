 ...
 * for more details.
 *
 * RCSID $Id: spdb.c,v 1.95 2004/02/19 15:54:28 dhr Exp $
 */
 ...
 * with members from { POLICY_ENCRYPT, POLICY_COMPRESS }
 * shifted right by POLICY_IPSEC_SHIFT.
 */
struct db_sa ipsec_sadb[((POLICY_ENCRYPT | POLICY_COMPRESS) >> POLICY_IPSEC_SHIFT) + 1] = {
    { AD_NULL },	/* none */
    { AD(esp_props) },	/* POLICY_ENCRYPT */
    { AD_NULL },	/* POLICY_COMPRESS */
...
, u_int8_t np)
{
     ...
     * (conjuncts) and must deal with different protocols (well, only ESP now).
	...
	for (pn = 0; pn != pc->prop_cnt; pn++)
	{
	    ...
	    int tn;
	    bool tunnel_mode = (pn == pc->prop_cnt-1);	/* only for last proposal */
	     ...
	     * Note: KLIPS doesn't use the protocol when looking
	     * up a (dest, protocol, spi).
		...
		else
		{
		     ...
		     * to Transport Mode, but we only want Tunnel
    ...
    struct state *st)		/* current state object */
{
	...
	struct isakmp_proposal esp_proposal, ipcomp_proposal;
	ipsec_spi_t esp_spi, ipcomp_cpi;
	bool esp_seen = FALSE, ipcomp_seen = FALSE;
	int inner_proto = 0;
	u_int16_t well_known_cpi = 0;

	pb_stream esp_trans_pbs, ipcomp_trans_pbs;
	struct isakmp_transform esp_trans, ipcomp_trans;
	struct ipsec_trans_attrs esp_attrs, ipcomp_attrs;
	 ...
	 * For now, we find an acceptable ESP.
	 */
	...
	if (esp_seen)
	{
		...
		if (esp_attrs.encapsulation != ENCAPSULATION_MODE_TUNNEL)
		    continue;	/* we only like tunnel mode: try another */
	...
	if (ipcomp_seen)
	{
		...
		if (esp_seen && esp_attrs.encapsulation != ipcomp_attrs.encapsulation)
		{
		    /* ??? This should be an error, but is it? */
		    DBG(DBG_CONTROL | DBG_CRYPT
			, DBG_log("ESP and IPCOMP transforms disagree about encapsulation (%s vs %s); TUNNEL presumed"
			    , enum_name(&enc_mode_names, esp_attrs.encapsulation)
			    , enum_name(&enc_mode_names, ipcomp_attrs.encapsulation)));
		}
