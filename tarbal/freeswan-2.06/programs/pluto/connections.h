 ...
 * for more details.
 *
 * RCSID $Id: connections.h,v 1.84 2004/02/05 17:16:27 mcr Exp $
 */
...
struct end {
    ...
    enum keyword_host host_addr_type;
    ip_address
	host_addr,
	host_nexthop;
    ip_subnet client;

    enum pubkey_source key_source;
    bool has_client;
    char *updown;
...
struct connection {
    ...
    enum connection_kind kind_wanted;   /* what the user asked for */
    enum connection_kind kind;          /* what we wind up with */
...
extern void initiate_opportunistic(const ip_address *our_client
    , const ip_address *peer_client, bool held, int whackfd, err_t why);
