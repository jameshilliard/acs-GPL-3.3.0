 ...
 * for more details.
 *
 * RCSID $Id: state.c,v 1.123 2004/01/15 17:42:09 mcr Exp $
 */
...
/*
 * protoid was relevant when AH was present.
 *
 */

struct state *
find_phase2_state_to_delete(const struct state *p1st
, u_int8_t protoid UNUSED
, ipsec_spi_t spi
, bool *bogus)
{
    struct state *st;
    ...
    *bogus = FALSE;
	...
	for (st = statetable[i]; st != NULL; st = st->st_hashchain_next)
	{
	    ...
	    {
		struct ipsec_proto_info *pr = &st->st_esp;
...
show_states_status(void)
{
    ...
    /* Note: the following code could be executed even if count == 0
     * but would trigger complaints from Electric Fence about malloc(0).
     * Besides, this way is more efficient.
     */
    if (count > 0)
    {
	/* build the array */
	struct state **array = alloc_bytes(sizeof(struct state *)*count, "state array");
