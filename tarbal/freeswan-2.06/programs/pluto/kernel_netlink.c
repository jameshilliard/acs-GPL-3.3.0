 ...
 * for more details.
 *
 * RCSID $Id: kernel_netlink.c,v 1.12 2004/02/19 15:54:28 dhr Exp $
 */
...
static sparse_names aalg_list = {
    { "digest_null", SADB_X_AALG_NULL },
    { "md5",         SADB_AALG_MD5HMAC },
    { "sha1",        SADB_AALG_SHA1HMAC },
    { "sha256",      SADB_X_AALG_SHA2_256HMAC },
    { "ripemd160",   SADB_X_AALG_RIPEMD160HMAC, },
    { sparse_end,    0 }
};

static sparse_names ealg_list = {
    { "cipher_null", SADB_EALG_NULL  },
    { "des",         SADB_EALG_DESCBC },
    { "des3_ede",    SADB_EALG_3DESCBC  },
    { "cast128",     SADB_X_EALG_CASTCBC },
    { "blowfish",    SADB_X_EALG_BLOWFISHCBC },
    { "aes",         SADB_X_EALG_AESCBC },
    { sparse_end,    0 }
};

static sparse_names calg_list = {
    { "deflate",     SADB_X_CALG_DEFLATE },
    { "lzs",         SADB_X_CALG_LZS },
    { "lzjh",        SADB_X_CALG_LZJH, },
    { sparse_end,    0 }
};
...
static bool
netlink_raw_eroute(const ip_address *this_host, const ip_subnet *this_client
, const ip_address *that_host, const ip_subnet *that_client
, ipsec_spi_t spi, unsigned int satype
, const struct pfkey_proto_info *proto_info
, time_t use_lifetime UNUSED
, unsigned int op, const char *text_said)
{
    struct {
	struct nlmsghdr n;
	union {
...
netlink_acquire(struct nlmsghdr *n)
{
	...
	record_and_initiate_opportunistic(&ours, &his, "%acquire-netlink");
...
netlink_shunt_expire(struct xfrm_userpolicy_info *pol)
{
    ...
    replace_bare_shunt(&src, &dst, SPI_PASS, FALSE, proto
...
, int proto
, bool tunnel_mode	/* NOTE: can only be true for last SPI of group */
, unsigned reqid
, ipsec_spi_t min
, ipsec_spi_t max
