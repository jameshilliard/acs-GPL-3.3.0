 ...
 * for more details.
 *
 * RCSID $Id: constants.c,v 1.88 2004/03/08 05:13:03 dhr Exp $
 */
...
prettypolicy(lset_t policy)
{
    ...
    if (shunt != 0)
    {
	snprintf(bitnamesbuf + len, sizeof(bitnamesbuf) - len, "%s%s"
	    , len == 0? "" : "+"
    ...
    if (fail != 0)
    {
	snprintf(bitnamesbuf + len, sizeof(bitnamesbuf) - len, "%sfailure%s"
	    , len == 0? "" : "+"
    ...
    {
	snprintf(bitnamesbuf + len, sizeof(bitnamesbuf) - len, "%sNEVER_NEGOTIATE"
	    , len == 0? "" : "+");
	len += strlen(bitnamesbuf + len);
    }
    return bitnamesbuf;
