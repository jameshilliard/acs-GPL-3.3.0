 ...
 * for more details.
 *
 * RCSID $Id: rsasigkey.c,v 1.27 2004/01/27 16:30:52 mcr Exp $
 */
...
/*
 * By default, we build keys of at least 1536 bits, and up to 4096 bits
 * with the exact number being random. .
 */
#ifndef IPSEC_KEY_MINIMUM
#define IPSEC_KEY_MINIMUM  1536
#endif
#ifndef IPSEC_KEY_MAXIMUM
#define IPSEC_KEY_MAXIMUM  4096
#endif
...
int vary = 0;                   /* generate random sized key */
unsigned long nbitsupper = IPSEC_KEY_MAXIMUM;
unsigned long nbitslower = IPSEC_KEY_MINIMUM;
...
int main(int argc, char *argv[])
{
		...
		case 'U':
		  {
		    ...
		    if(colon) {
		      ugh = ttoul(optarg, colon-optarg, 0, &nbitslower);
		      
		      if(ugh) {
			fprintf(stderr,
				"key size lower must be a number, not %s: %s\n",
				optarg, ugh);
			exit(2);
		      }
		    ...
		    ugh = ttoul(optarg, 0, 0, &nbitsupper);
		    if(ugh) {
		      fprintf(stderr,
			      "key size upper must be a number, not %s: %s\n",
			      optarg, ugh);
		      exit(2);
		    }
		  }
	...
	if (errflg
	    || (oldkeyfile != NULL && optind != argc)
	    || (oldkeyfile == NULL && vary == 0 && optind != (argc-1))
	    || (vary == 1 && optind != argc)
	    || (vary == 1 && oldkeyfile != NULL))
	{
	    if(vary && oldkeyfile != NULL) {
		printf("%s: --vary and --oldkeyfile are mutually exclusive\n",
		       argv[0]);
	    }		
	    printf("Usage:\n%s\n", usage);
	    printf("%s\n", usage2);
...
{
    unsigned int nbits;
    u_int32_t vary, range;
    char buf[128];

    /* get 32 bit number from random device */
    getrandom(sizeof(vary), (char *)&vary);

    /* figure out range that it is supposed to be in */
    range = nbitsupper - nbitslower;

    /* and add it, modulo the range */
    nbits = nbitslower + (vary % range);

    /* we need it to be a multiple of 16 bits, so axe lower 4 bits */
    nbits = (nbits + 0x10) & ~0xF;

    snprintf(buf, sizeof(buf), "Picked RSA key of size %u bits (%u random)\n",
	     nbits, vary);
    report(buf);
...
#	define	OKAY(p)	(eval == 0 || mpz_fdiv_ui(p, eval) != 1)
	...
	while (!( OKAY(var) && mpz_probab_prime_p(var, nrounds) )) {
		...
		tries++;
		if (verbose && (tries & 0xff)==0) {
		    fprintf(stderr, "still looking after %lu tries.\n", tries);
		}
	}
...
char *buf;			/* known to be big enough */
{
	...
	/*
	 * clear the buffer, so that if the read fails, we can tell 
	 * how far it got, in the debugger
	 */
	memset(buf, 0, nbytes);
	...
	/* test randomness by counting 1s and 0s */
	{
	    int sum;

	    sum = parityofbuffer(buf, nbytes);

	    /* XXX this formula needs to have more thought */
	    /* it was 10* abs(sum), but that was too sensitive */
	    /* someone needs to do some more research here */
	    if(abs(sum) >= nbytes) {
		fprintf(stderr,
			"%s: WARNING when asked for %d bytes, we found it was non-random by %d\n",
			me, nbytes, sum);
	    }
	}
}
