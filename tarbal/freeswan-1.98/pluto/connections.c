 ...
 * for more details.
 *
 * RCSID $Id: connections.c,v 1.124.2.4 2002/06/21 19:22:47 mcr Exp $
 */
...
static void initiate_opportunistic_body(const ip_address *our_client
    , const ip_address *peer_client, bool held, int whackfd);	/* forward */
...
continue_oppo(struct adns_continuation *acr, err_t ugh)
{
	...
	, is_held =  
#ifdef KLIPS
	    has_bare_hold(our_client, peer_client);
#else
    ...
    if (ugh != NULL)
    {
	cannot_oppo(our_client, peer_client, is_held, ugh);
    }
    ...
    {
	passert(gateways_from_dns != NULL);
	initiate_opportunistic_body(our_client, peer_client, is_held, whackfd);
