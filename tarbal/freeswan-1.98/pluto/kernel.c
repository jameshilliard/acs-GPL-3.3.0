 ...
 * for more details.
 *
 * RCSID $Id: kernel.c,v 1.149.2.2 2002/06/13 18:37:29 mcr Exp $
 */
...
show_shunt_status(void)
{
	...
	whack_log(RC_COMMENT, "%s -> %s => %s", ourst, hist, sat);
    }
}
...
bool
has_bare_hold(const ip_address *src, const ip_address *dst)
{
    ...
    bspp = bare_shunt_ptr(&this_client, &that_client);
    return bspp != NULL
