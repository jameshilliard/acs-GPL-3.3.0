 ...
 * for more details.
 *
 * RCSID $Id: adns.h,v 1.2.2.1 2002/06/04 01:28:33 mcr Exp $
 */
...
#define ADNS_Q_MAGIC (((((('d' << 8) + 'n') << 8) + 's') << 8) + 2)
#define ADNS_A_MAGIC (((((('d' << 8) + 'n') << 8) + 's') << 8) + 128 + 2)
...
struct adns_query {
    size_t len;
    unsigned int qmagic;
    lset_t impairments;	/* for testing */
#	define ADNS_IB_DELAY_ANSWER	LELEM(0)	/* delay answer for this query */
